...
def parse_arguments(args):
    ...
    if args.loggingconf is not None:
        ...
        logging_conf = os.path.join(
            os.path.dirname(os.path.abspath(sys.argv[0])), "logging.conf"
        )
...
def download_csv(log, download_url, raw_csv_fpath):
    ...
    raw_csv_red = requests.get(download_url, allow_redirects=True)
...
def preprocess_table(log, raw_csv_fpath, data_csv_fpath, git_dir_data, job_dir):
    ...
    start_year, start_month, start_day = regex.search(
        covid19_data["Meldedatum"].min()
    ).groups()
    end_year, end_month, end_day = regex.search(
        covid19_data["Meldedatum"].max()
    ).groups()
    ...
    if sum(df.sum() == 0):
        raise RuntimeError(
            "A county without any Covid cases in dataframe: CSV Parsing Error"
        )
...
def submit_job(log, slurm_jobfile, submit_dir, sbatch_addargs=""):
    ...
    if slurm_jobid:
        with open(os.path.join(submit_dir, slurm_jobfile + ".jobid"), "w") as f:
